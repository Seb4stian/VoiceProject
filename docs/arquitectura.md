...
5. [Componentes del sistema](#5-componentes-del-sistema)
   ...
   - [Hojas de estilo — `static/css/`](#56-hojas-de-estilo--staticcss)
   - [Prompts de IA — `prompts/`](#57-prompts-de-ia--prompts)
   - [Grabaciones — `recordings/`](#58-grabaciones--recordings)
   - [Configuración — `config.json`](#59-configuración--configjson)
...
│   ├── __init__.py
│   ├── recorder.py      # Subida, listado, eliminación y servicio de grabaciones.
...
│       ├── chat.css         # Estilos del chat de psicología.
│       ├── voice_chat.css   # Estilos del chat de voz.
...
`app.py` implementa el patrón **Application Factory**: la función `create_app()` instancia Flask, importa y registra los siete blueprints de rutas, y devuelve la aplicación lista para ser ejecutada.
...
### 5.6 Hojas de estilo — `static/css/`
...
- Las grabaciones pueden eliminarse individualmente mediante el endpoint `DELETE /recordings/<filename>`, que también es accesible desde el botón **Delete** en la interfaz de la grabadora.
...
Los estilos de cada página se mantienen en hojas de estilo externas dentro de `static/css/` en lugar de embebidos en las plantillas HTML. Esto separa la presentación de la estructura, facilita la colaboración entre diseñadores y desarrolladores, y permite al navegador cachear los estilos de forma independiente, mejorando el rendimiento en visitas sucesivas.
